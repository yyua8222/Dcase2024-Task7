...
class DDIMSampler(object):
    def __init__(self, model, schedule="linear", device=torch.device("cuda"), extra_channels = False ,**kwargs):
        super().__init__()
        self.model = model
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if len(x.shape)==3:
            ...
            sqrt_one_minus_at = torch.full(
                (b, 1, 1), sqrt_one_minus_alphas[index], device=device
            )
        else:
            a_t = torch.full((b, 1, 1, 1), alphas[index], device=device)
            a_prev = torch.full((b, 1, 1, 1), alphas_prev[index], device=device)
        ...
        if self.extra_channels:
            x_prev = torch.cat([x_prev,x_extra],dim = 1)
            
            
        return x_prev, pred_x0
    
    @torch.no_grad()
    def encode(self, sample, steps= 200,eta = 0.0, verbose = True, encode_step = None):
        ...
        if encode_step is None:
            encode_step = steps


        timesteps = self.ddim_timesteps

        time_range = timesteps[:encode_step]

        total_steps = time_range.shape[0]
        

        iterator = tqdm(time_range, desc="DDIM encoder", total=total_steps)
        ...
        for i, t in enumerate(iterator):
            ...
            if prev_timestep>=0:
                alpha_prod_t_prev = self.model.alphas_cumprod[prev_timestep]
            else:
            ...
            cur_step =  torch.full((1,), t, dtype=torch.long).cuda()

            # ipdb.set_trace()
            model_output = self.model.apply_model(sample, cur_step)
            # ipdb.set_trace()
