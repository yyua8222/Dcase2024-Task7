...
from clap.training.params import parse_args
import braceexpand
import numpy as np
import pandas as pd
import torch
...
from clap.open_clip.utils import get_tar_path_from_dataset_name, dataset_split
...
tokenize = RobertaTokenizer.from_pretrained("roberta-base",cache_dir = "pre_load_models")
...
def get_audio_features(
    ...
    # print(audio_data.size(-1))
    # print(max_len)
    # print(str(audio_data.size()))
    # assert audio_data.size(-1) > max_len, str(audio_data.size())
    # split to three parts
    chunk_frames = int(max_len // audio_cfg["hop_size"] + 1)  # the +1 related to how the spectrogram is computed
